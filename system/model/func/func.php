...
function updateGadgets($table = "", $colname = "", $objname = "", $colname2 = "", $objname2 = "",  $colname3 = "", $objname3 = "", $condColName = "", $condobjname = "")
{
	global $db;
	$colname = $db->SQLEscape($colname);
	$objname = $db->SQLEscape($objname);
	...
	$sql  = "UPDATE {$table}";
	$sql .= " SET  {$colname} = '{$objname}', ";
	$sql .= " {$colname2} = '{$objname2}', ";
	$sql .= " {$colname3} = '{$objname3}' ";
	$sql .= " WHERE {$condColName} = '{$condobjname}' ";
	$sql .= "LIMIT 1";
...
function updateStatus($token_id = "", $owner = "", $status = "")
{
	global $db;
	...
	$sql  .= "status = '{$status}' ";
	$sql .= " WHERE `tokon_id` ='{$token_id}' AND `walletId` = '{$owner}' ";
	$sql .= "LIMIT 1";
...
function addtoblockchain($id = "", $walletId = "")
{
	global $db;
	$id = $db->SQLEscape($id);
	$walletId = $db->SQLEscape($walletId);



	$sql  = "UPDATE `gadgets` SET `is_added`=true WHERE `id`='{$id}' AND `walletId`='{$walletId}'";
...
function SearchForPhones($input = "")
{
	global $db;
...
function fetch_open_phones()
{
	global $db;
	// $type = $db->SQLEscape($type);
	$sql = "SELECT * FROM `gadgets` WHERE `is_lock`=0 ORDER BY `price` DESC";
	$rsArray = $db->query($sql);
...
function fetch_phones_by_walletid($walletId = "")
{
	global $db;
	// $type = $db->SQLEscape($type);
	$sql = "SELECT * FROM `gadgets`WHERE `walletId`='{$walletId}' ORDER BY `price` DESC";
	$rsArray = $db->query($sql);
...
function fetch_user_info($walletId = "")
{
	global $db;
	// $type = $db->SQLEscape($type);
	$sql = "SELECT * FROM `user_info`WHERE `walletId`='{$walletId}' LIMIT 1";
	$rsArray = $db->query($sql);
...
function pending_order($walletId = "")
{
	global $db;
	// $type = $db->SQLEscape($type);
	$sql = "SELECT * FROM `gadgets`WHERE `walletId`='{$walletId}' OR `lockBy`='{$walletId}' AND `status`='2' ORDER BY `price` DESC";
	$rsArray = $db->query($sql);
...
function order_histroy($walletId = "")
{
	global $db;
	// $type = $db->SQLEscape($type);
...
{
	global $db;
	$sql = "SELECT `full_info` FROM `models` WHERE `model`='{$model}'";
	$rsArray = $db->query($sql);
...
function findGadgetByCustodians($value = "")
{
	global $db;
...
function findListingBySlug($listing_slug = "")
{
	global $db;

	$listing_slug = $db->SQLEscape($listing_slug);
	// $sql = "SELECT * FROM `gadgets` WHERE `slug`='{$listing_slug}'";
