<?php 
    class Submit_list extends Controller
    {
        ...
        if ($_FILES["file1"] && $_FILES["file2"] && $_FILES["file3"] && $_FILES["file4"] && $_FILES["file5"]) {
            $imgsurfix = "DRV";
            ...
            if (in_array($ext1, $valid_extensions) && in_array($ext2, $valid_extensions) && in_array($ext3, $valid_extensions) && in_array($ext4, $valid_extensions) && in_array($ext5, $valid_extensions)) {
                ...
                // $msgArray = array('status' => "OK", 'msg' => $img_url);
                if (empty($category) || 
                    empty($sub_category) || 
                    empty($brand) || 
                    empty($condition) || 
                ...
                // $img_url = $img_url.strtolower($image); 
